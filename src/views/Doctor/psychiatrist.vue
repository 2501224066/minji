<template>
  <div class="doctor_list">
    <!-- 搜索框 -->
    <Search :bgc="'#eff4f5'" :bg="'#fff'" :price_active="this.price_active" :city_right="this.city_right"
      :m_active="this.m_active" :department_right="this.department_right" @search="search" @change="change"></Search>
    <!-- 筛选 -->
    <div class="select_box">
      <van-dropdown-menu active-color="#4f9ec9">
        <van-dropdown-item :title="city_text" ref="city" id="city">
          <van-tree-select :items="city_arry" :active-id.sync="city_right" :main-active-index.sync="city_left"
            :height="500" @click-item="city_code" />
        </van-dropdown-item>
        <van-dropdown-item :title="department_text" ref="department">
          <van-tree-select :items="department_arry" :active-id.sync="department_right"
            :main-active-index.sync="department_left" :height="500" @click-item="department_code" />
        </van-dropdown-item>
        <van-dropdown-item :title="mechanism_text" ref="mechanism">
          <div class="mechanism_list">
            <div :class="['mechanism_item', m_active == item.id ? 'mechanism_active' : '']"
              v-for="(item) in mechanism_list" :key="item.id" @click="choose_mechanism(item)">
        ...
        <van-dropdown-item title="筛选" ref="screen">
          <div class="screen_box">
            <div class="screen_price">
              ...
              <div class="price_box">
                <span :class="['price_item', price_active == item.id ? 'price_active' : '']" @click="choose_price(item)"
                  v-for="item in price_list" :key="item.id">
                  {{ item.text
                  }}
                </span>
              </div>
    ...
    <div class="content_box">
      <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad"
        ...
        <div class="doctor_item" @click="go_detail(item.id)" v-for="item in doctor_arry" :key="item.id">
          ...
          <div class="doctor_content">
            <div class="content_one">
              ...
              <div class="label_box">
                <i class="label" v-if="index <= 0" v-for="(label, index) in item.hospital.tag" :key="label">
                  {{
                    label
                  }}
                </i>
...
export default {
  ...
  data() {
    return {
      ...
      department_right: "10",
      department_arry: [
        { text: "心理科室", children: [{ text: "心理科室", id: 10 }] }
      ],
      department_text: "心理科室",
